...
namespace ScheduleI.DispensariesMod.Integration
{
    public sealed class ScheduleIGameApi : IGameApi
    {
        ...
        public IRuntimeActions Runtime { get; private set; }
        ...
        public ScheduleIGameApi(object gameRoot, Action<string> infoLog, Action<string> warnLog, Action<string, Exception> errorLog)
        {
            ...
            Runtime = new ReflectionRuntimeActions(gameRoot, Log);
        }

        private sealed class DelegateLogApi : ILogApi
        {
            private readonly Action<string> _info; private readonly Action<string> _warn; private readonly Action<string, Exception> _error;
        ...
        private sealed class ReflectionProgressionApi : IProgressionApi
        {
            ...
            public bool TryGetMayorWeedLegalizationFlag(out string resolvedFlag, out bool value) { foreach (var flag in CandidateFlags) { if (TryGetFlag(flag, out value)) { resolvedFlag = flag; return true; } } resolvedFlag = string.Empty; value = false; _log.Warn("No mayor legalization progression flag found."); return false; }
        }
        ...
        private sealed class ReflectionRealEstateApi : IRealEstateApi { private readonly object _root; private readonly ILogApi _log; public ReflectionRealEstateApi(object root, ILogApi log) { _root = root; _log = log; } public bool IsAvailable { get { return ReflectionLookup.CanHookRealEstate(_root, _log); } } public event Func<string, bool> OnPurchaseRequested = delegate { return false; }; public void RegisterDynamicListingProvider(string categoryId, Func<IList<RealEstateListing>> provider) { ReflectionLookup.RegisterRealEstateProvider(_root, categoryId, provider, delegate (string id) { return OnPurchaseRequested(id); }, _log); } }
        private sealed class ReflectionInventoryApi : IInventoryApi { private readonly object _root; private readonly ILogApi _log; public ReflectionInventoryApi(object root, ILogApi log) { _root = root; _log = log; } public IList<PlayerProductStack> GetCannabisProducts() { return ReflectionLookup.GetCannabisProducts(_root, _log); } public bool TryRemove(string productId, int quantity) { return ReflectionLookup.TryRemoveInventory(_root, productId, quantity, _log); } public bool TryAddDebugProduct(string productId, string productName, ProductGrade grade, int quantity, decimal basePrice) { return ReflectionLookup.TryAddInventory(_root, productId, productName, grade, quantity, basePrice, _log); } }
        ...
        private sealed class ReflectionDiagnostics : IIntegrationDiagnostics { private readonly object _root; private readonly ILogApi _log; public ReflectionDiagnostics(object root, ILogApi log) { _root = root; _log = log; } public IntegrationStatus GetStatus() { return new IntegrationStatus { LegalizationFlagLookupFound = ReflectionLookup.HasLegalizationFlagAccess(_root, _log), RealEstateHookFound = ReflectionLookup.CanHookRealEstate(_root, _log), UiHookFound = ReflectionLookup.CanOpenUiPanels(_root, _log), SaveHookFound = ReflectionLookup.CanRegisterSave(_root, _log), DailyTickFound = ReflectionLookup.CanHookDailyTick(_root, _log) }; } public void DumpStatus() { var s = GetStatus(); _log.Info("Integration status => Flag:" + s.LegalizationFlagLookupFound + " RealEstate:" + s.RealEstateHookFound + " UI:" + s.UiHookFound + " Save:" + s.SaveHookFound + " DailyTick:" + s.DailyTickFound); } }
        private sealed class ReflectionRuntimeActions : IRuntimeActions { private readonly object _root; private readonly ILogApi _log; public ReflectionRuntimeActions(object root, ILogApi log) { _root = root; _log = log; } public bool IsHotkeyPressed(string keyName) { return ReflectionLookup.IsHotkeyPressed(_root, keyName, _log); } public void ShowMayorLegalizationMessage() { ReflectionLookup.ShowMayorLegalizationMessage(_root, _log); } public void DumpIl2CppTypeCandidates() { ReflectionLookup.DumpIl2CppTypeCandidates(_root, _log); } }
    }
}
